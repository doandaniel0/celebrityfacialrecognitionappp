    ...
    if 'upload_button' in request.form:
        try:
            ...
            if file and allowed_file(file.filename) and file.filename != '':
                ...
                output = upload_file_to_s3(file, 'uploadtomatch')
                
                # Retrieve and display Rekognition results
                results = search_collection('uploadtomatch', file.filename, 'collectiondatabase')
                if results:
                    ...
                    # Display celeb image
                    celeb_path = "{}{}".format('http://{}.s3.amazonaws.com/'.format('collectiondatabase'),
    ...
    if 'double_upload_button' in request.form:
        try:
            ...
            if (file1 and allowed_file(file1.filename) and file1.filename != '' and
                ...
                output1 = upload_file_to_s3(file1, 'uploadtomatch')
                output2 = upload_file_to_s3(file2, 'uploadtomatch')
            ...
            error = 'Two images must be select hk hj gh1``   ted'
...
@application.route('/face', methods=['POST', 'GET'])
def upload_detect_faces():
    error = None
    if 'upload_button' in request.form:
        try:
            file = request.files['user_file']
        ...
        except: error = 'No objects found in image'

                if response:
                    for faceDetail in response['FaceDetails']:
                        flash(faceDetail['AgeRange']['Low'] + ': ' + "{:.2f}".format(faceDetail['AgeRange']['High']),
                              'status')
    ...
    return render_template('face.html', error=error)
